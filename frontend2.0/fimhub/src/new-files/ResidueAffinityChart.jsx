...
export default function ResidueAffinityChart({
  residueGroups,
  selectedPosition,
  onSelectResidue,
  deltaColor,
}) {
  // Calculate average dAffinity for each residue
  const data = residueGroups.map((group) => {
    const affinities = group.records
      .map((r) => r.dAffinity)
      .filter((v) => v != null);
    const avgAffinity =
      affinities.length > 0
