      ...
      <div className="max-h-[85vh] w-full max-w-4xl overflow-y-auto border border-[color:var(--fh-border)] bg-[color:var(--fh-surface)] p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h3 className="mt-3 text-3xl text-[color:var(--fh-text)]">
              {mutationId}
            </h3>
            <p className="mt-3 text-sm leading-7 text-[color:var(--fh-text-secondary)]">
              Quantitative comparison of the selected substitution across every
              curated model where it is present.
            </p>
          </div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {entries.map((entry) => (
            <div
              key={entry.modelId}
              className="border border-[color:var(--fh-border)] p-5"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[color:var(--fh-accent)]">
                    {entry.species}
                  </p>
                  <h4 className="mt-2 truncate text-2xl text-[color:var(--fh-text)]">
                    {entry.displayName}
                  </h4>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                <div className="border-t border-[color:var(--fh-border)] pt-3">
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[color:var(--fh-text-secondary)]">
                    Affinity (M)
                  </p>
                  <p className="mt-3 truncate font-mono text-lg font-bold text-[color:var(--fh-text)]">
